# Stops an app using docker-compose
if [ "$1" == "--help" ] || [ -z "$1" ]; then
  echo "Usage: app stop <app> [--environment ENVIRONMENT]]"
  echo "Stops an app using docker-compose"
...
if [ "$(docker-compose -f $DOCKER_COMPOSE_PATH ps | grep 'Up' | awk '{print $1}')" ]; then
